...
`script.js` is the canonical source. `script.min.js` and `style.min.css` are generated by a local pre-commit hook (`minify.sh`, gitignored) using [terser](https://terser.org/). Never edit the minified files directly.
...
The pre-commit hook runs `minify.sh` (also gitignored) which:
1. Minifies `script.js` → `script.min.js` via terser
2. Minifies `style.css` → `style.min.css` via a custom Node script
3. Auto-stages the generated files

To set up locally:
```bash
npm install -g terser
# Create minify.sh and .git/hooks/pre-commit (see project docs)
```
