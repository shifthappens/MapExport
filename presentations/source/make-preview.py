...
import zipfile, xml.etree.ElementTree as ET, posixpath, sys
root=Path(__file__).resolve().parents[1]
ns={'a':'http://schemas.openxmlformats.org/drawingml/2006/main'}
deck=sys.argv[1] if len(sys.argv)>1 else 'mapexport-workshop-merged.pptx'
with zipfile.ZipFile(root/deck) as z:
    ...
    for slide in ET.fromstring(z.read('ppt/presentation.xml')).findall('.//{http://schemas.openxmlformats.org/presentationml/2006/main}sldId'):
        target=posixpath.normpath(posixpath.join('ppt',rels[slide.attrib['{http://schemas.openxmlformats.org/officeDocument/2006/relationships}id']]))
        slide_rels=ET.fromstring(z.read(posixpath.join(posixpath.dirname(target),'_rels',posixpath.basename(target)+'.rels')))
        note_target=next(r.attrib['Target'] for r in slide_rels if r.attrib['Type'].endswith('/notesSlide'))
        r=ET.fromstring(z.read(posixpath.normpath(posixpath.join(posixpath.dirname(target),note_target))))
...
parts[0]=parts[0].replace('href="mapexport-workshop.pptx"',f'href="{escape(deck,quote=True)}"')
