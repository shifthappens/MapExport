...
8. **Live real-world export + faithful visual check** (standard verification of actual SVG output — and a real test: it exits non-zero on failure):
   ```
   node tests/real-export.mjs [city|s,w,n,e] [--record] [--engine=v2] [--illustrator]
   ```
   ...
   `--engine=v2` routes construction through the experimental `engine-v2.js` instead (see `ENGINE-V2.md`); v2 runs additionally enforce the coverage promise via `coverage-lint.mjs` (model-side) and `render-coverage.mjs` (rendered ink in headless Chrome — the authority).
...
- `layer-selection.mjs` — offline v1/v2 layer-panel contract: every checkbox removes its SVG layer, disabled transit does not fetch, and v2 fetches private block-construction data only while City blocks is selected.
...
- `pin-cache.mjs` — offline check of `tools/pin-cache.sh`, which owns `cache/pinned/`. Builds a throwaway repo (a copy of the real script plus a stub prefetcher standing in for Overpass) and asserts the three permanent mistakes it must not make: pinning an entry that is not a whole gzip stream and whole JSON, replacing a good pin with a half-written file, and "refreshing" without actually refetching. Also covers the stash: a failed refresh restores what it parked, a partial one keeps what it fetched, and a stash left by a killed run is recovered rather than left to overwrite newer data.
...
- `v2-face-runtime-benchmark.mjs` — focused real-worker parity check for PERF-01 through PERF-05: compares the retained unoptimized reference path with the optimized path on synthetic rural, urban, off-frame water/landcover and hamlet-reach fixtures. It requires exact serialized parity where promised, exact visible in-frame parity where off-frame ink may be culled, and prints worker phase timings without a timing threshold.
