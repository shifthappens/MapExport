/* @ds-bundle: {"format":4,"namespace":"MapExportDesignSystem_14c71d","components":[],"sourceHashes":{"script.js":"915886f3ea98"},"inlinedExternals":[],"unexposedExports":[]} */

(() => {

const __ds_ns = (window.MapExportDesignSystem_14c71d = window.MapExportDesignSystem_14c71d || {});
...
const LAYER_REGISTRY = [{
  ...
  layers: [{
    ...
    id: 'parks',
    label: 'Parks & green',
    hint: 'Named parks, forests, reserves',
    color: '#b8d89a',
    defaultOn: true,
    type: 'area',
    fillOpacity: 1,
    ...
    tagFilter: el => {
      ...
      return /park|nature_reserve|recreation_ground/.test(el.tags?.leisure || '') || el.tags?.landuse === 'forest' || el.tags?.natural === 'wood';
    }
  }]
}, {
  ...
  layers: [{
    id: 'water_labels',
    label: 'Water & park names',
    hint: 'Rivers, lakes, parks',
    color: '#1a3a6a',
    defaultOn: true,
    type: 'feature_labels',
...
const TYPE_LABELS = {
  motorway: 'Motorways',
  trunk: 'Trunk roads',
  motorway_link: 'Motorway links',
  trunk_link: 'Trunk links',
...
function renderLabelToggles() {
  ...
  cats.forEach(cat => {
    ...
    label.querySelector('input').addEventListener('change', e => {
      LABEL_VISIBILITY[cat] = e.target.checked;
      scheduleLivePreview();
    });
...
function startDraw() {
  ...
  function onDown(e) {
    ...
    function onMove(ev) {
      ...
      bboxRect = L.rectangle([drawStart, ev.latlng], {
        color: '#bf3b1e',
        weight: 1.5,
        fillColor: '#bf3b1e',
        fillOpacity: 0.07,
    ...
    function onUp(ev) {
      ...
      hideToast();
      const s = Math.min(drawStart.lat, ev.latlng.lat),
        n = Math.max(drawStart.lat, ev.latlng.lat);
      const w = Math.min(drawStart.lng, ev.latlng.lng),
        ea = Math.max(drawStart.lng, ev.latlng.lng);
...
function updateBboxDisplay() {
  ...
  if (!bbox) {
    d.innerHTML = '<div>No area selected yet</div>';
  ...
  const latSpan = north - south,
    lngSpan = east - west;
  const kmNS = (latSpan * 111).toFixed(1),
    kmEW = (lngSpan * 111 * Math.cos((north + south) / 2 * Math.PI / 180)).toFixed(1);
...
async function fetchBoundaries(placeName) {
  ...
  try {
    ...
    const data = await (await fetch(url, {
      headers: {
        'Accept-Language': 'en',
        'User-Agent': 'MapExport/1.0'
      }
    })).json();
    res.innerHTML = '';
    if (!data.length) {
      res.innerHTML = '<div style="padding:6px 10px;font-size:9.5px;color:var(--muted)">No boundaries found</div>';
      return;
    }
  ...
  for (let s = s0; s < bbox.north; s = +(s + TILE_SIZE).toFixed(10)) {
    for (let w = w0; w < bbox.east; w = +(w + TILE_SIZE).toFixed(10)) {
      tiles.push({
        s: +s.toFixed(1),
        w: +w.toFixed(1),
        n: +(s + TILE_SIZE).toFixed(1),
        e: +(w + TILE_SIZE).toFixed(1)
      });
    }
  }
  return tiles;
...
async function fetchLayer(layer, bboxStr, bbox) {
  ...
  return {
    elements: mergeElements(elementArrays),
    failedTiles
  };
}
...
function makeProjector(b, W) {
  const [xMin, yMin] = degToMerc(b.west, b.south),
    [xMax, yMax] = degToMerc(b.east, b.north);
  const scale = W / (xMax - xMin);
  const H = Math.round((yMax - yMin) * scale);
  function pr(lat, lng) {
    const [mx, my] = degToMerc(lng, lat);
    return [(mx - xMin) * scale, H - (my - yMin) * scale];
  }
  return {
    pr,
    H
...
function dpSimplify(pts, eps) {
  ...
  if (maxD > eps) {
    const l = dpSimplify(pts.slice(0, idx + 1), eps),
      r = dpSimplify(pts.slice(idx), eps);
...
function elementInBbox(el, b) {
  ...
  if (el.bounds) {
    return el.bounds.maxlat >= b.south && el.bounds.minlat <= b.north && el.bounds.maxlon >= b.west && el.bounds.minlon <= b.east;
  }
  // Node (POI)
  if (el.type === 'node') {
  ...
  if (el.members?.length) {
    return el.members.some(m => m.geometry?.some(g => g.lat >= b.south && g.lat <= b.north && g.lon >= b.west && g.lon <= b.east));
  }
...
// ════════════════════════════════════════════════════════════════
function safeName(s) {
  return (s || '').replace(/&/g, 'and').replace(/[<>"']/g, '').replace(/\s+/g, '_').replace(/[^\w\-]/g, '_').slice(0, 80);
}
function escXml(s) {
...
function buildMetroLayer(elements, pr, W) {
  ...
  lineMap.forEach(line => {
    if (!line.color) line.color = METRO_PALETTE[pi++ % METRO_PALETTE.length];
  });
  let lineGroups = '';
...
function approxTextWidth(t, fs, ls = 0) {
  return t.length * (fs * 0.65 + ls);
}
...
function makeCollisionGrid() {
  const placed = [];
  return {
    overlaps(cx, cy, w, h, pad = 4) {
      const hw = w / 2 + pad,
        hh = h / 2 + pad;
      for (const r of placed) if (Math.abs(cx - r.cx) < hw + r.hw && Math.abs(cy - r.cy) < hh + r.hh) return true;
      return false;
    },
    add(cx, cy, w, h, pad = 4) {
      placed.push({
...
function buildLabelsLayer(elements, pr, W, H) {
  ...
  sorted.forEach(el => {
    ...
    const name = el.tags.name,
      hw = el.tags.highway || '_default';
    if (!roundaboutGroups.has(name)) roundaboutGroups.set(name, {
      hw,
      elements: []
    });
    roundaboutGroups.get(name).elements.push(el);
  });
  roundaboutGroups.forEach(({
    hw,
    elements
  }, name) => {
  ...
  sorted.forEach(el => {
    ...
    const isClosed = pts.length >= 3 && Math.hypot(pts[0][0] - pts[pts.length - 1][0], pts[0][1] - pts[pts.length - 1][1]) < 2;
    ...
    const lh = sz * 1.4,
      pad = Math.abs(Math.sin(angle * Math.PI / 180)) * lh;
...
function buildFeatureLabelsLayer(elements, pr, W, H) {
  ...
  elements.forEach(el => {
    ...
    if (place === 'suburb' || place === 'neighbourhood' || place === 'quarter') {
      ...
      sz = natural === 'water' ? 24 * sf : 22 * sf;
      weight = 400;
      color = natural === 'water' ? '#3a6a9a' : '#3a6a3a';
    } else return;
    const tw = approxTextWidth(name, sz),
      th = sz * 1.4;
...
function getBlockWorkerUrl() {
  if (!blockWorkerUrl) {
    blockWorkerUrl = URL.createObjectURL(new Blob([BLOCK_WORKER_SRC], {
...
function prepareBlockData(allResults, pr, W, H) {
  ...
  const BLOCK_ROADS = new Set(['motorway', 'trunk', 'primary', 'secondary', 'tertiary', 'residential', 'unclassified', 'living_street', 'pedestrian', 'motorway_link', 'trunk_link', 'primary_link', 'secondary_link', 'tertiary_link']);
  ...
  for (const {
    ...
    if (layer.id === 'parks' || layer.id === 'water_bodies') {
      for (const el of data.elements) {
        const geoms = el.type === 'way' ? [el.geometry] : el.type === 'relation' && el.members ? el.members.map(m => m.geometry) : [];
...
function computeBlocksAsync(allResults, pr, W, H, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    if (!data.lines.length && !data.areas.length) {
      resolve([]);
      return;
    }
...
function buildSVG(results, b, W, physicalWidthMm = null, precomputedBlocks = null) {
  ...
  sorted.forEach(({
    ...
    if (layer.type === 'roads') {
      layersSVG += buildRoadsLayer(elements, pr, W);
      return;
    }
    if (layer.type === 'rail') {
      layersSVG += buildRailLayer(elements, pr, W);
      return;
    }
    if (layer.type === 'metro') {
      layersSVG += buildMetroLayer(elements, pr, W);
      return;
    }
    if (layer.type === 'tram') {
      layersSVG += buildTramLayer(elements, pr, W);
      return;
    }
    if (layer.type === 'labels') {
      layersSVG += buildLabelsLayer(elements, pr, W, H);
      return;
    }
    if (layer.type === 'feature_labels') {
      layersSVG += buildFeatureLabelsLayer(elements, pr, W, H);
      return;
    }
    ...
    if (layer.id === 'water_bodies' || layer.id === 'waterways') {
      fillColor = preset.water;
      strokeColor = preset.water;
    }
    if (layer.id === 'parks') {
      ...
      elements.forEach(el => {
        ...
        if (el.type === 'relation' && el.members) {
          el.members.forEach(m => {
            d += geomToPathD(m.geometry, pr, EPS.area_large, true) + ' ';
          });
          d = d.trim();
        }
        if (!d) return;
    ...
    if (layer.id === 'buildings' && precomputedBlocks && precomputedBlocks.length) {
      ...
      strokeColor = preset.buildingStroke;
    ...
    elements.forEach(el => {
      ...
      if (el.type === 'relation' && el.members) el.members.forEach(m => {
        allD += geomToPathD(m.geometry, pr, eps, isArea) + ' ';
      });
    });
    let content = '';
    const d = allD.trim();
...
function getAllSelectedLayers() {
  ...
  LAYER_REGISTRY.forEach(g => g.layers.forEach(l => {
    if (document.getElementById('lyr-' + l.id)?.checked) layers.push(l);
  }));
  return layers;
}
async function doExport() {
  ...
  for (let t = 0; t < tiles.length; t++) {
    ...
    updateProgress(`Fetching tile ${t + 1}/${tiles.length} — ${uncachedLayers.length} layer${uncachedLayers.length > 1 ? 's' : ''}…`, Math.round(t / tiles.length * 85));
    ...
    for (const layer of uncachedLayers) {
      const elements = layer.tagFilter ? combined.elements.filter(layer.tagFilter) : combined.elements;
      layerElements[layer.id].push(...elements);
      cacheSet(tileCacheKey(layer.id, tile), {
        elements
      });
    }
    fetchedTiles++;
  ...
  const results = selected.map(layer => ({
    layer,
    data: {
      elements: mergeElements([layerElements[layer.id]]),
      failedTiles: []
    }
  }));
...
function renderHistory() {
  try {
    ...
    items.forEach(entry => {
      ...
      const kmNS = ((north - south) * 111).toFixed(0),
        kmEW = ((east - west) * 111 * Math.cos((north + south) / 2 * Math.PI / 180)).toFixed(0);
      ...
      div.querySelector('.hi-del').addEventListener('click', e => {
        e.stopPropagation();
        try {
          const h = JSON.parse(localStorage.getItem('mapexport_history') || '[]');
          localStorage.setItem('mapexport_history', JSON.stringify(h.filter(x => x.id !== entry.id)));
          renderHistory();
        } catch (e) {}
      });
      div.addEventListener('click', () => {
        bbox = entry.bbox;
        map.fitBounds([[entry.bbox.south, entry.bbox.west], [entry.bbox.north, entry.bbox.east]], {
          padding: [20, 20]
        });
        if (bboxRect) map.removeLayer(bboxRect);
        bboxRect = L.rectangle([[entry.bbox.south, entry.bbox.west], [entry.bbox.north, entry.bbox.east]], {
          color: '#bf3b1e',
          weight: 1.5,
          fillColor: '#bf3b1e',
          fillOpacity: 0.07,
...
async function searchCity(query) {
  ...
  resEl.innerHTML = '';
  try {
    const data = await (await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=json&limit=5&addressdetails=1`, {
      headers: {
        'Accept-Language': 'en',
        'User-Agent': 'MapExport/1.0'
      }
...
function showFailedTileOverlays(tiles, layerLabel) {
  ...
  tiles.forEach(tile => {
    const rect = L.rectangle([[tile.s, tile.w], [tile.n, tile.e]], {
      color: '#bf3b1e',
      weight: 1,
      fillColor: '#e03020',
      fillOpacity: 0.25,
      dashArray: '4 3',
      className: 'failed-tile-rect'
    });
    rect.bindTooltip(`<span class="failed-tile-tooltip">Failed: ${layerLabel}</span>`, {
      permanent: false,
      direction: 'top',
      className: 'failed-tile-tooltip-wrap'
    });
...
function showFailedTileSummary(count) {
  ...
  banner.innerHTML = `<span>${count} tile${count > 1 ? 's' : ''} failed — highlighted in red on map</span>` + `<button id="btn-retry-failed" class="failed-tile-retry-btn">Retry</button>` + `<button id="btn-dismiss-failed" class="failed-tile-dismiss-btn">✕</button>`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('search-input').addEventListener('input', e => {
    ...
    if (e.target.value.length > 2) searchTimeout = setTimeout(() => searchCity(e.target.value), 500);
  });
  document.addEventListener('click', e => {
    if (!e.target.closest('#search-wrap')) document.getElementById('search-results').classList.remove('show');
    if (!e.target.closest('.panel')) document.getElementById('boundary-results').classList.remove('show');
  });

  // Boundary button
  document.getElementById('btn-boundary').addEventListener('click', () => {
    const q = document.getElementById('search-input').value;
    if (q.trim()) fetchBoundaries(q);else setStatus('Type a city name first, then click Use admin boundary', 'error');
  });
  document.getElementById('btn-draw').addEventListener('click', startDraw);
  document.getElementById('btn-export').addEventListener('click', doExport);
  document.getElementById('btn-dl').addEventListener('click', () => {
    if (lastSvgString) triggerDownload(lastSvgString, lastSvgFilename);
  });
